...
This is an interactive web application built with [Shiny for Python](https://shiny.posit.co/py/) that provides a toolkit for financial analysis. The app loads the `sample.parquet` dataset (created by the data pipeline) and offers three main modules:
...
👉🏻 Try it now: [Portfolio, Sector & Event Lab] (https://ozkangelincikshinyapp.shinyapps.io/yfinance-app1/)
